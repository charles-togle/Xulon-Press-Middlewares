...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data } = await supabase.from("fact_contacts").select(
      "ghl_contact_id",
    ).eq("ghl_contact_id", ghlBody.contact_id).maybeSingle();
    let contactExists;
    if (data) {
      contactExists = true;
    } else {
    ...
    if (eventType === "contact-update") {
      await handleUpdateContact({ ghlBody: ghlBody, supabase: supabase });
      return new Response(
        JSON.stringify({
          ok: true,
    ...
    if (eventType === "opportunity-update") {
      await handleUpdateOpportunity({ ghlBody: ghlBody, supabase: supabase });
      return new Response(
        JSON.stringify({
          ok: true,
