import { createGhlNote, createGhlOpportunity, updateFactContactTable } from "./utils.ts";
const handleCreateContact = async ({ ghlBody, LOCATION_ID, supabase })=>{
  const { data: supabase_contact, error: supabase_contact_error } = await supabase.rpc("insert_contact_to_star_schema", {
    // Person
  ...
  await createGhlNote(einstein_notes_payload, ghlBody.contact_id);
