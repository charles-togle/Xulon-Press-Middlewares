...
interface HandleUpdateContactParams {
    supabase: SupabaseClient;
    ghlBody: GhlBody;
}

const handleUpdateContact = async (
    { supabase, ghlBody }: HandleUpdateContactParams,
) => {
    const { error } = await supabase.from(
        "fact_contacts",
    ).select("ghl_opportunity_id").eq("ghl_contact_id", ghlBody.contact_id);
    if (error) {
    ...
    const existingOpportunityData = await searchGhlOpportunity(
        ghlBody.contact_id,
    );
    ...
    const { data: pipelineNames, error: pipelineError } = await supabase.rpc(
    ...
    const { data: supabase_contact, error: supabase_contact_error } =
        await supabase.rpc("update_contact_in_star_schema_using_contact_id", {
            ...
            p_stage_id: pipelineStageId ??
                "b9ccd7f2-2d77-43a2-9e0c-a59388fd5c3e",
    ...
    const opportunityInfo = await updateGhlOpportunity(
